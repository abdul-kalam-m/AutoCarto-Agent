...
| C6 Tiered compute router, V-W simplification | `compute_router.py` | ❌ | `visvalingamwyatt` pinned but never imported; no PostGIS/Sedona code — unchanged, Phase 3+ scope |
| C7 bbox-first hybrid retrieval | `hybrid_retrieval.py` | ✅ | mock-verified incl. antimeridian |
| C7′ ST_Intersects exact refinement | — | ❌ | envelope test only — unchanged, Phase 3 scope |
| C7″ Metadata scorer (7-point TRUSTED/AUGMENT/REJECT), profiler | — | ❌ | `metadata_score` is a stored int in mocks; no scoring logic — unchanged, Phase 3 scope |
| C8 Atlanta 530-tract case | `gen_results_panel.py`, `demo_data.py` | ✅ | reproduced exactly; also now reachable offline via `autocarto run` (2026-07-27) |
...
**Bottom line (updated 2026-07-27):** 13 of 21 rows are now ✅ (was 5 of 21). The remaining ❌/⚠️ rows are consistently Phase 3+ (Data Fabric completion, real-data benchmark) and Phase 5 (container security) scope — exactly what §11's roadmap always said would come later. The core validation gate suite (C4, all six gates) and the orchestrator (C1) — the system's namesake — are no longer aspirational.
