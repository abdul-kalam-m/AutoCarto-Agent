...
| Test coverage ≥80% on `gates/`, `sandbox` | ✅ Measured 2026-09-16 with `pytest --cov=autocarto`: gates 92.79% (592/638), sandbox 84.30% (188/223); overall 87.78%. Statement coverage, not branch coverage. Baseline: 316 passed, 30 skipped. Command and per-module evidence: [coverage baseline](../docs/coverage-baseline.md). | P1 |
| All 6 gates implemented | ✅ 6/6 (2026-07-27) | P1 |
| Deterministic trace schema + diff tool | ✅ Phase 0, 2026-09-16: packaged [schema.json](../src/autocarto/traces/schema.json) covers workspace v2, web trace v1, and new orchestrator trace v1. `python -m autocarto.traces validate/diff` validates and compares records; imports recompute against exact dataset hashes. Legacy audit/demo JSON remains historical, not silently migrated. [Format and compatibility](../docs/workspace-format.md). | P2 |
