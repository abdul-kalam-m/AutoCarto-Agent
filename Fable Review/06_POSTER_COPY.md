...
> `pip install -e . && autocarto demo` → statistical traces **byte-identical** across runs · fully offline · core validation < 3 s · 67 tests · pinned TIGER snapshot (SHA-256)
...
| Byte-identical traces · offline · core validation <3 s · 67 tests | Block C | `autocarto demo` twice + `pytest` — **read the "Total wall-clock" line the tool itself prints**, not a stopwatch on the shell command |
...
**Three things this pass corrected that earlier drafts got wrong** (listed most-serious first — a false claim baked into image pixels, not just document prose):
...
*Copy drafted against the verified state at commit `a1fdb0a`; wording fixes at `2b9b749`; end-to-end regeneration and the two corrections above from the 2026-07-26 pass (§10). If the benchmark or figures are regenerated with different seeds/corpus before printing, re-run §9 and §10 and update the affected blocks — never the other way around.*
