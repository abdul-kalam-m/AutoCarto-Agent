...
> - **Classification.** Naïve breaks on a skewed variable collapse **414 of 530 tracts (78%) into one class** — the map renders fine and says nothing. *→ Gate 2*
...
> **AutoCarto-Agent removes the LLM's statistical authority instead of improving its judgment.** The LLM parses intent, selects visual variables and templates, and assembles code. A deterministic engine computes every number, vetoes invalid proposals, and — the key move — **rejects with a prescription**: the mandated method, the precomputed break values, splice-ready code. Rejection with prescription converges in ≤3 iterations, because the LLM's remaining job is transcription. Every decision lands in a machine-readable trace.
...
> Container isolation — gVisor, network-none *(designed)*
...
> **Same variable, same 530 tracts.** Ungated: equal-interval breaks collapse 78% of tracts into one class — a flat wash. Gated: Gate 2 diagnoses the skew and prescribes log+Jenks — balanced classes **[98 / 134 / 144 / 91 / 63]** reveal the gradient. *Every number computed by the shipping Gate 2; regression-tested.*
...
> Both variables heavy-right-skew → Gate 2 vetoes naïve Jenks, prescribes **log-transform + Jenks**
> **GVF: 0.75 → 0.83** (canopy) and **0.77 → 0.86** (asthma) over naïve quintiles
...
> **95% correct verdicts** (20/21 strict): 6/6 benign passed, 14/15 pathological rejected — **every rejection with its prescription**
> The single miss — two independent fields spuriously cross-correlating — is disclosed; a conditional-permutation null is queued.
...
*Rationale:* Block A's GVF line replaces the untraceable "raises GVF from failure to 0.894" with the verified before/after (Manual §5). Block B leads with **decision accuracy against known truth** — the defensible headline the synthetic corpus actually supports — and *prints the miss*, which converts your weakest point into evidence of rigor before anyone asks Q12. Block C is the claim no poster neighbor can match; keep it terse and typeset like code.
...
*Rationale:* Guide §4.2-5 — poster sessions reward candor; this box answers "only two gates?" (Q2) before it's asked and makes the uniform gate stack in the architecture band honest.
...
| 95% (20/21) · 6/6 benign · 14/15 pathological · 1 disclosed miss | Block B | `autocarto benchmark` |
