...
1. **F-NEW-1 — "Ungated vs Gated", the killer visual (~1 day).** Same Atlanta data, two maps side by side: LEFT = what a naive pipeline renders (quantile breaks on the skewed variable, rainbow/red-green ramp, Web-Mercator, no legend annotation) with three red callouts naming each violation and the gate that catches it; RIGHT = the AutoCarto output with green PASS badges. All ingredients exist (`gen_results_panel.py` + Gate 2). *This is the single highest-leverage artifact you can add* — it makes the entire thesis visible in one glance and belongs in the poster's bottom-left whitespace and as Slide 2's payoff.
2. **F-NEW-2 — Trace excerpt panel (~1 hour).** A styled JSON snippet: the actual Gate-2 REJECT with `prescribed_breaks` and instruction text ("DO NOT propose alternative methods"), next to the revised-code diff. Reviewers trust artifacts over diagrams.
3. **F-NEW-3 — Rejection-flow Sankey** (proposals → G1..G6 outcomes → prescribe → converge/HITL). **Only after** the §8 mini-benchmark exists — do not fabricate flows.
