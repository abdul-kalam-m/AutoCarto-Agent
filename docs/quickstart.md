...
**Running `--runtime=runsc` for real needs a runner with root/privileged access** — `runsc install` writes to `/etc/docker/daemon.json` and restarts the Docker daemon, and gVisor itself has kernel-feature and container-nesting requirements beyond that. GitHub's hosted `ubuntu-latest` runners happen to provide this (they're full VMs, not restricted containers), which is why the `gvisor-security` CI job works there without extra configuration. It will **not** work unmodified on a locked-down self-hosted runner, a restricted Kubernetes-based CI system, or inside another container/dev-container without `--privileged` and (depending on the host) an alternate gVisor platform (`--platform=systrap` instead of the ptrace-based default, which itself often can't nest inside another ptrace-based sandbox). If `runsc install` or the CI job fails in an environment like that, treat it as an environment/deployment mismatch to resolve deliberately, not a software bug — the red-team suite (`tests/security/test_escapes.py`) skips cleanly rather than failing when `runsc` genuinely isn't available, which is the correct behavior for a dev machine or an unprivileged CI runner.
