...
Exercises the four core modules with synthetic but realistic data,
...
Run (after ``pip install -e .``):
    autocarto demo [--out DIR]        # default DIR = ./output
    python -m autocarto.demo [--out DIR]

The statistical content of the emitted traces is byte-identical across
runs (seeded everywhere); only wall-clock timing fields vary.
"""
...
from autocarto.data_fabric.hybrid_retrieval import HybridRetrieval, STACItem
from autocarto.execution.sandbox import CodeSanitizer, _DevOnlySandboxExecutor


# Output paths — bound by _init_dirs() before any demo function runs.
# Import of this module must stay side-effect free (no dir creation here).
OUT_ROOT: Path
FIG_DIR: Path
TRACE_DIR: Path
LOG_DIR: Path
...
def _init_dirs(out_root: Path) -> None:
    """Bind the module-level output paths and create the directories."""
    global OUT_ROOT, FIG_DIR, TRACE_DIR, LOG_DIR
...
def main(argv: List[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="autocarto demo",
        description="Run the deterministic AutoCarto-Agent demo harness.",
    )
    parser.add_argument(
